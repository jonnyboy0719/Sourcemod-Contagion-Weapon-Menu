...
#define sNotice "\x01[\x04Notice\x01]"
#define sError "\x01[\x04Error\x01]"
...
new String:g_sWeaponEntityMelee[MAX_CONFIG_WEAPONS][32];
...
public Plugin:myinfo = {
	name = "[Contagion] Weapon Menu",
	author = "Marcus, Modified by JonnyBoy0719",
...
{
	CreateConVar("sv_contagion_weaponmenu_version", Plugin_Version, "This is the version of the Weapon Menu the server is running.", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY);
	g_hPluginEnabled = CreateConVar("sv_contagion_weaponmenu_enabled", "1", "Turns the plugin on or off. (1 = Enabled : 0 = Disabled)", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 1.0);
	HookConVarChange(g_hPluginEnabled, OnSettingsChange);

	g_hRifleLimit = CreateConVar("sv_weaponmenu_rifle_limit", "1", "Sets the maximum amount of times a rifle can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hRifleLimit, OnSettingsChange);
	g_hSubMachineGunLimit = CreateConVar("sv_weaponmenu_smg_limit", "1", "Sets the maximum amount of times a smg can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hSubMachineGunLimit, OnSettingsChange);
	g_hShotgunLimit = CreateConVar("sv_weaponmenu_shotgun_limit", "1", "Sets the maximum amount of times a shotgun can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hShotgunLimit, OnSettingsChange);
	g_hPistolLimit = CreateConVar("sv_weaponmenu_pistol_limit", "1", "Sets the maximum amount of times a pistol can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hPistolLimit, OnSettingsChange);
	g_hToolLimit = CreateConVar("sv_weaponmenu_tool_limit", "1", "Sets the maximum amount of times a tool can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hToolLimit, OnSettingsChange);
	g_hMeleeLimit = CreateConVar("sv_weaponmenu_melee_limit", "1", "Sets the maximum amount of times a melee weapon can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hMeleeLimit, OnSettingsChange);
	g_hMedicalLimit = CreateConVar("sv_weaponmenu_medical_limit", "10", "Sets the maximum amount of times medical supplies can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hMedicalLimit, OnSettingsChange);
	g_hExplosiveLimit = CreateConVar("sv_weaponmenu_explosive_limit", "1", "Sets the maximum amount of times explosives can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hExplosiveLimit, OnSettingsChange);
	g_hWeaponAdminOverride = CreateConVar("sv_weaponmenu_admin_override", "1", "Discards all weapon limitations for an Admin.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
...
public Action:Command_WeaponMenu(iClient, iArgs)
{
	if (IsPlayerAlive(iClient))
	{
		...
		SetMenuTitle(hMenu, "[Contagion] Weapon Menu");
...
public Weapon_Menu(Handle:hHandle, MenuAction:action, param1, param2)
{
	switch (action)
	{
		...
		case MenuAction_Select:
		{
			...
			Format(output, sizeof(output), "melee_script_name %s::::1", g_sWeaponEntityMelee[StringToInt(sInfo)]);
...
Config_LoadWeapons() // Originally, this was from TwistedPanda's BuildWars : It helped alot!!
{
	...
	if (FileToKeyValues(hKeyValue, sPath))
	{
		...
		do
		{
			...
			KvGetString(hKeyValue, "entity_melee", g_sWeaponEntityMelee[g_iWeapons], sizeof(g_sWeaponEntityMelee[]));
		...
		PrintToServer("[Contagion] Weapon Menu: Loaded %d weapons", g_iWeapons);
	} else
	{
		CloseHandle(hKeyValue);
		SetFailState("[Contagion] Weapon Menu: Could not locate: %s", sPath);
	}
}
